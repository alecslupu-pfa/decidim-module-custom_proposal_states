...
describe "Proposals in process home", type: :system do
  ...
  context "when there are proposals" do
    let!(:proposals) { create_list(:extended_proposal, proposals_count, component: component) }
    let!(:drafted_proposals) { create_list(:extended_proposal, proposals_count, :draft, component: component) }
    let!(:hidden_proposals) { create_list(:extended_proposal, proposals_count, :hidden, component: component) }
    let!(:withdrawn_proposals) { create_list(:extended_proposal, proposals_count, :withdrawn, component: component) }
    ...
    context "and there are more proposals than those that can be shown" do
      let!(:proposals) { create_list(:extended_proposal, highlighted_proposals + 2, component: component) }
