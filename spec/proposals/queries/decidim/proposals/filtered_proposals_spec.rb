...
describe Decidim::Proposals::FilteredProposals do
  ...
  let(:another_component) { create(:extended_proposal_component, participatory_space: participatory_process) }

  let(:proposals) { create_list(:extended_proposal, 3, component: component) }
  let(:old_proposals) { create_list(:extended_proposal, 3, component: component, created_at: 10.days.ago) }
  let(:another_proposals) { create_list(:extended_proposal, 3, component: another_component) }
